...
public class GptUtil {
            ...
            try (CloseableHttpClient httpClient = httpClientBuilder.setSSLSocketFactory(getSslConnectionSocketFactory()).build()) {
                ...
                if (code == 200) {
                    ...
                    while ((j = reply.indexOf("cmd(", i)) != -1) {
                        ...
                        if (flag && server.getCommands().performPrefixedCommand(commandSourceStack, command) != 0) {
    ...
    public static void sendMessageToEveryone(MinecraftServer server, String msg, String sender) {
        ...
        for (String name : playerNames) {
            ...
            if (serverPlayer != null) {
                for (String str : split) {
                    if (!str.trim().equals("")) {
                        serverPlayer.displayClientMessage(MutableComponent.create(new LiteralContents("<" + sender + "> " + str)), false);
                    }
                }
            }
        }
        for (String str : split) {
            if (!str.trim().equals("")) {
                server.sendSystemMessage(MutableComponent.create(new LiteralContents("<" + sender + "> " + str)));
            }
        }
    }
}
